...
if __name__ == "__main__":
    ...
    # Visualization of t-SNE transformation for training data
    plt.figure(figsize=(8, 6))
    plt.scatter(train_data[:, 0], train_data[:, 1], c=train_targets, cmap='viridis', s=10)
    plt.colorbar(label='Digit')
    plt.title('t-SNE Visualization of MNIST Training Dataset')
    plt.xlabel('Component 1')
    plt.ylabel('Component 2')
    plt.show()

    # Train your model with t-SNE transformed data
    model = customMLP(2, hidden_size, output_size)  # Input size changed to reflect t-SNE components
